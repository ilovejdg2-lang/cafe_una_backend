...
export class VisitasService {
  ...
  async crear(datos: Partial<VisitaGrupal>): Promise<VisitaGrupal> {
    ...
    if (tipoVisitante.toLowerCase() === 'internacional' && !datos.PaisProcedencia?.trim()) {
      throw new BadRequestException(
        'El país de procedencia es obligatorio para visitantes internacionales.',
      );
    }

    if (!datos.EncargadoNombre?.trim() || !datos.EncargadoEmail?.trim() || !datos.EncargadoTelefono?.trim()) {
      throw new BadRequestException('Los datos del encargado (nombre, correo y teléfono) son obligatorios.');
    }

    if (!datos.FechaVisita?.trim() || !datos.HoraPreferida?.trim()) {
      throw new BadRequestException('La fecha solicitada y el bloque de hora son obligatorios.');
    }
  ...
  async obtenerSolicitudes(filtros: {
    ...
    if (filtros.estado?.trim()) {
      qb.andWhere('LOWER(v.Estado) = LOWER(:estado)', { estado: filtros.estado.trim() });
    }
    ...
    if (filtros.fechaDesde?.trim()) {
      qb.andWhere('v.FechaVisita >= :fechaDesde', { fechaDesde: filtros.fechaDesde.trim() });
    }

    if (filtros.fechaHasta?.trim()) {
      qb.andWhere('v.FechaVisita <= :fechaHasta', { fechaHasta: filtros.fechaHasta.trim() });
    }
  ...
  async actualizar(id: string, cambios: Partial<VisitaGrupal>): Promise<VisitaGrupal> {
